...
function Upcoming_stream(props) {
    const { openEditModel, scheduledStream, deleteStream, handleEditClose, handleEditOpen, editData, setFormData, handleSubmit, register, errors } = props

    const API_URL = import.meta.env.VITE_API_URL || `http://localhost:3000`
    const handleComplete = async (id) => {
        try {
            const {response} = await axios.post(`${API_URL}/stream/complete-stream/${id}`)
